...
#include <pmonitor/pmonstate.h>
//#include "pmondisplay.h"
//#include "pmongui.h"

#include <TThread.h>
#include <TGClient.h>
#include <pmonitor/pMutex.h>
#include <pthread.h>
  ...
  while (evt )
    {
      ...
      if ( ( nevents > 0 && ++ncount >= nevents) ){
        cout << "pstart(" << nevents << ") finished." << endl;
        break;
      }
      evt = theIterator->getNextEvent();
    }
...
void phelp()
{
  ...
  cout << " phDraw_Detector_Energy(Id number, Rebin, Same/NoSame)        Draw the energy histogram of a specific detector, a rebin value. Third argument is a string. If = ""same"" -> Draw(""same"") " << endl;
  cout << " phDraw_Detector_Energy(Id number, Rebin)        Draw the energy histogram of a specific detector and a rebin value" << endl;
  cout << " phDraw_Detector_Energy(Id number)        Draw the energy histogram of a specific detector" << endl;
  cout << " phDraw_Detector_Rate(Id number, Rebin, Same/NoSame)        Draw the rate histogram of a specific detector, a rebin value. Third argument is a string. If = ""same"" -> Draw(""same"") " << endl;
  cout << " phDraw_Detector_Rate(Id number, Rebin)        Draw the rate histogram of a specific detector and a rebin value" << endl;
  cout << " phDraw_Detector_Rate(Id number)        Draw the rate histogram of a specific detector" << endl;
  cout << " pOpenCanvas()        Open the main canvases" << endl;
  cout << " pstop()        Stop reading the data stream" << endl;
