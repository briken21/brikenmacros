...
// Gamma canvas----------------------------------------------------------
  ...
  Gamma_Energy_Canvas->Divide(3,3);
...
//Canvas_QuickRate
Canvas_QuickRate = new TCanvas("Canvas_QuickRate","Quick rate Canvas",900,900);
Canvas_QuickRate->SetFillColor(10);
Canvas_QuickRate->Divide(1,2);
Canvas_QuickRate->Draw();
Canvas_QuickRate->cd(1);
HRate_3HeTotal->Draw("histo");
Canvas_QuickRate->cd(2);
HRate1D_IndividualDetector[148]->Draw("histo");
//----------------------------------------------------------------------
...
void Init(){
    ...
    HEnergy_3HeTotal = new TH1F("HEnergy_3HeTotal","Energy 3He Total ; Energy(keV) ; Counts",600,0,3000);
    // TH1 He3 rings
    for (int imn=0; imn<7; imn++){
    ...
    for (int imk=1; imk<vName.size(); imk++){
      ...
      if (vInputType[imk]==3)HEnergy1D_IndividualDetector[imk-1] = new TH1F(Form("HEnergy1D_Id%d",imk),HistoNameEnergy.c_str(),20000,0,20000);
    ...
    //HRate_3HeTotal->SetLabelSize(0.05,"xyz");
    // He3 Ring
    for (int imn=0; imn<7; imn++){
    ...
    for (int imk=1; imk<vName.size(); imk++){
      ...
      //HRate1D_IndividualDetector[imk-1]->SetLabelSize(0.05,"xyz");
...
//Neutron distribution refresh------------------------------------
      ...
      for (int imn=0; imn<2; imn++){
        Canvas_QuickRate->cd(imn+1);
        gPad->Modified();
        Canvas_QuickRate->Update();
      }
    ...
    if (data_now->b==8||data_now->b==9||data_now->b==10){
      for (Int_t i=0;i<V1740_N_MAX_CH;i++){// Loop per all the 64 channels from 1740 modules
        ...
        if (hit->clong*vEFactor[IdChannel_Trigger]+vEOffset[IdChannel_Trigger]>vThreshold[IdChannel_Trigger]){// Set threshold in keV.
          ...
          HEnergy_3HeTotal->Fill(hit->clong*vEFactor[IdChannel_Trigger]+vEOffset[IdChannel_Trigger],1);
          HEnergy1D_IndividualDetector[IdChannel_Trigger-1]->Fill(hit->clong*vEFactor[IdChannel_Trigger]+vEOffset[IdChannel_Trigger],1);
          if (vInputType[IdChannel_Trigger]==1)HEnergy3He_Ring[vIndex1[IdChannel_Trigger]-1]->Fill(hit->clong*vEFactor[IdChannel_Trigger]+vEOffset[IdChannel_Trigger],1);
          //------------------------------------------
          //Rate histograms
...
int SaveHistograms(){
  ...
  if (file0){
    ...
    // Canvas Quick rate
    if (Canvas_QuickRate) Canvas_QuickRate->Write();
    //--------------------
