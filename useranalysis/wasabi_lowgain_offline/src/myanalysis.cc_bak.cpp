...
UShort_t trig_pos = N_MAX_WF_LENGTH*20/100;//unit of sample
...
void Init(){
    for (Int_t i=0;i<V1740_N_MAX_CH*8;i++){
        hwf2d[i]=new TH2F(Form("hwf2d%d",i),Form("hwf2d%d",i),400,0,400,500,0,5000);
    ...
    for (Int_t i=0;i<4;i++){
        ...
        hstripmult2d[i] = new TH2F(Form("hstripmult2d%d",i),Form("hstripmult2d%d",i),32,0,32,32,0,32);
    }
    ...
    hbl2d = new TH2F("hbl2d","hbl2d",V1740_N_MAX_CH*8,0,V1740_N_MAX_CH*8,500,0,5000);
    //hbl2d->Draw("colz");
    he2d->Draw("colz");
    pupdate(c1,2);
...
void ProcessEvent(NIGIRI* data_now){
    if (data_now->b<8&&data_now->b>=0){
        ...
        Double_t esumx = 0;
        Double_t esumy = 0;
        Int_t dssdno = data_now->b-4;
        Int_t xmult=0,ymult=0;
        //data_now->Print();
        for (Int_t i=0;i<V1740_N_MAX_CH;i++){
            ...
            if (hit->clong>0){
                ...
                hbl2d->Fill(ch,hit->baseline);

                //! strip multiplicities
                if (i<32){//X strips
                    if (hit->clong>exmax) {
                        exmax = hit->clong;
                        xmax = i;
                    ...
                    esumx+=hit->clong;
                    if (hit->clong+hit->baseline==4095)
                    //if (hit->clong>200)
                            xmult++;
                }else{//Y strips
                    if (hit->clong>eymax) {
        ...
        hstripmult2d[dssdno]->Fill(xmult,ymult);
...
void DoUpdate(){
    ...
    hrateupdate->Reset();
    if (nupdate%5==0&&nupdate>0) hbl2d->Reset();
    nupdate++;
    //pstatus();
}
