...
Int_t dgtz_b;
Long64_t dgtz_ts;
UShort_t dgtz_nsample[V1740_N_MAX_CH];
Double_t dgtz_e[V1740_N_MAX_CH];
Double_t dgtz_bl[V1740_N_MAX_CH];
Double_t dgtz_t[V1740_N_MAX_CH];
Int_t dgtz_ch[V1740_N_MAX_CH];
UShort_t dgtz_sample[V1740_N_MAX_CH][N_MAX_WF_LENGTH];
UShort_t dgtz_waveform[V1740_N_MAX_CH][N_MAX_WF_LENGTH];
...
Double_t           dssd_ex[N_STRIP_X];
Double_t           dssd_exbl[N_STRIP_X];
Double_t           dssd_tx[N_STRIP_X];
Int_t           dssd_mutlx;
Long64_t           dssd_ts_x;
Long64_t           dssd_ts_y;
Double_t           dssd_ey[N_STRIP_X];
Double_t           dssd_eybl[N_STRIP_X];
Double_t           dssd_ty[N_STRIP_X];
Int_t           dssd_mutly;

Double_t           dgzt_bl_low[N_DSSD][V1740_N_MAX_CH];
Double_t           dgzt_bl_up[N_DSSD][V1740_N_MAX_CH];
...
void CloseEvent(){
    ...
    Double_t sumex = 0;
    Double_t sumey = 0;
    for (int xy=0;xy<V1740_N_MAX_CH;xy++){
        if (xy<N_STRIP_X){//x strips
            dssd_ex[xy] = dgtz_e[xy];
            dssd_tx[xy] = dgtz_t[xy];
            dssd_exbl[xy] = dgtz_e[xy]+dgtz_bl[xy];
            if (!(dgtz_e[xy]>0&&dgtz_bl[xy]>dgzt_bl_low[dgtz_b][xy]&&dgtz_bl[xy]<dgzt_bl_up[dgtz_b][xy])){
                continue;
            }
            dssd_mutlx++;
            sumex+=dssd_ex[xy];
            ...
            dssd_ey[xy-N_STRIP_X] = dgtz_e[xy];
            dssd_ty[xy-N_STRIP_X] = dgtz_t[xy];
            dssd_eybl[xy-N_STRIP_X] = dgtz_e[xy]+dgtz_bl[xy];
            if (!(dgtz_e[xy]>0&&dgtz_bl[xy]>dgzt_bl_low[dgtz_b][xy]&&dgtz_bl[xy]<dgzt_bl_up[dgtz_b][xy])){
                continue;
            }
            dssd_mutly++;
            sumey+=dssd_ey[xy-N_STRIP_X];
            if (dssd_ey[xy-N_STRIP_X]>ey){
                ey = dssd_ey[xy-N_STRIP_X];
                y = xy-N_STRIP_X;
                dssd_ts_y = dgtz_ts;
            }
        }
...
void ProcessEvent(NIGIRI* data_now){
    if (data_now->b >= 0 && data_now->b <= 3) {
        dgtz_b = data_now->b;
        dgtz_ts=data_now->ts;
        for (Int_t i=0;i<data_now->GetMult();i++){
            NIGIRIHit* hittemp=data_now->GetHit(i);
            int ch=hittemp->ch;
...
void OpenFile(const char* filename){
    ...
    tree->Branch("dgtz_ts",&dgtz_ts,"dgtz_ts/L");
    tree->Branch("dgtz_b",&dgtz_b,"dgtz_b/I");
    tree->Branch("dgtz_e",dgtz_e,Form("dgtz_e[%d]/D",V1740_N_MAX_CH));
    tree->Branch("dgtz_bl",dgtz_bl,Form("dgtz_bl[%d]/D",V1740_N_MAX_CH));
    tree->Branch("dgtz_t",dgtz_t,Form("dgtz_t[%d]/D",V1740_N_MAX_CH));
    tree->Branch("dgtz_ch",dgtz_ch,Form("dgtz_ch[%d]/I",V1740_N_MAX_CH));
    tree->Branch("dgtz_nsample",dgtz_nsample,Form("dgtz_nsample[%d]/s",V1740_N_MAX_CH));
    tree->Branch("dgtz_waveform",dgtz_waveform,Form("dgtz_waveform[%d][%d]/s",V1740_N_MAX_CH,N_MAX_WF_LENGTH));
    tree->Branch("dgtz_sample",dgtz_sample,Form("dgtz_sample[%d][%d]/s",V1740_N_MAX_CH,N_MAX_WF_LENGTH));
    ...
    tree->Branch("dssd_ts_x",&dssd_ts_x,"dssd_ts_x/L");
    tree->Branch("dssd_ts_y",&dssd_ts_y,"dssd_ts_y/L");

    tree->Branch("dssd_ex",dssd_ex,Form("dssd_ex[%d]/s",N_STRIP_X));
    tree->Branch("dssd_exbl",dssd_exbl,Form("dssd_exbl[%d]/D",N_STRIP_X));
    tree->Branch("dssd_tx",dssd_tx,Form("dssd_tx[%d]/D",N_STRIP_X));

    tree->Branch("dssd_ey",dssd_ey,Form("dssd_ey[%d]/D",N_STRIP_X));
    tree->Branch("dssd_eybl",dssd_eybl,Form("dssd_eybl[%d]/D",N_STRIP_X));
    tree->Branch("dssd_ty",dssd_ty,Form("dssd_ty[%d]/D",N_STRIP_X));
    tree->Branch("dssd_mutly",&dssd_mutly,"dssd_mutly/I");
    ClearEvent();
}
...
const pmap_decode packetdecode[]={NONE,V1740ZSP,V1740ZSP,V1740ZSP,V1740ZSP,NONE,NONE,NONE,NONE,NONE,NONE,NONE,NONE,NONE};
