...
TH2F *he2d;
TH1F *he1d[V1730_MAX_N_CH*MAX_N_V1730_BOARD];
TH1F *cloverrate[V1730_MAX_N_CH*MAX_N_V1730_BOARD];

int nevt = 0;
Long64_t start_ts[V1730_MAX_N_CH*MAX_N_V1730_BOARD];

TCanvas * c1;
TCanvas * c2;

void Init(){
    for (Int_t ch=0;ch<V1730_MAX_N_CH*MAX_N_V1730_BOARD;ch++){
        he1d[ch]=new TH1F(Form("he1d%d",ch),Form("he1d%d",ch),20000,0,20000);
        cloverrate[ch]=new TH1F(Form("cloverrate%d",ch),Form("cloverrate%d",ch),3600,0,3600);
        start_ts[ch] = 0;
    }
    ...
    he2d = new TH2F("he2d","he2d",V1730_MAX_N_CH*MAX_N_V1730_BOARD,0,V1730_MAX_N_CH*MAX_N_V1730_BOARD,10000,0,20000);
    he2d->Draw("colz");
...
void ProcessEvent(NIGIRI* data_now){
    ...
    if (data_now->b==12){
        ...
        if (data_now->ts>0&&hit->clong>150){
            if (start_ts[ch]==0) start_ts[ch] = data_now->ts;
            Long64_t tscurr_ns = (Long64_t)data_now->ts - start_ts[ch];
            Double_t tscorr_s = (Double_t) tscurr_ns/1e9;
            cloverrate[ch] ->Fill(tscorr_s);
        }
...
void CloseMe(){
    if (file0) {
        ...
        file0 = new TFile("online.root","recreate");
        for (Int_t ch=0;ch<V1730_MAX_N_CH*MAX_N_V1730_BOARD;ch++){
            he1d[ch]->Write();
            cloverrate[ch]->Write();
        }
