...
TH1F *he1dclover[V1730_MAX_N_CH];

TH2F *he_e[4];
TH2F *he_e_strip_high[4];
TH2F *he_e_strip_low[4];
...
std::multimap <ULong64_t,CORR *> datamap_hit1; //! sort by timestamp
std::multimap <ULong64_t,CORR *> datamap_hit2; //! sort by timestamp
std::multimap<ULong64_t,CORR *>::iterator it_datamap_hit1;
std::multimap<ULong64_t,CORR *>::iterator it_datamap_hit2;
...
void Init(){
    treedata = new NIGIRI;
    for (Int_t i=0;i<64*5;i++){
        hwf2d[i]=new TH2F(Form("hwf2d%d",i),Form("hwf2d%d",i),300,0,300,500,0,5000);
    }
    he2d = new TH2F("he2d","he2d",64*5,0,64*5,4000,0,4000);
    ...
    for (Int_t ch=0;ch<V1730_MAX_N_CH;ch++){
        he1dclover[ch]=new TH1F(Form("he1dclover%d",ch),Form("he1dclover%d",ch),4000,0,2000);
    }
    hcorrcloverSi = new TH1F("hcorrcloverSi","hcorrcloverSi",500,-20000,20000);
    ...
    for(int i = 0; i < 4; i++){
        he_e[i]=new TH2F(Form("he_e%d",i),Form("Correlation matrix DSSD %d with Clover detector",i),2000,0,4000,4000,0,2000);
        he_e_strip_low[i]=new TH2F(Form("he_e_strip_low%d",i),Form("Correlation matrix with Low stat. strip (ch0 from 0), dssd %d",i),2000,0,4000,4000,0,2000);
        he_e_strip_high[i]=new TH2F(Form("he_e_strip_high%d",i),Form("Correlation matrix with High stat. strip (ch31 from 0), dssd  %d",i),2000,0,4000,4000,0,2000);

        c1->cd(i+1);
        he_e[i]->Draw("colz");
        c2->cd(i+1);
        he_e_strip_low[i]->Draw("colz");
        c3->cd(i+1);
        he_e_strip_high[i]->Draw("colz");
    }
...
//    tree->Fill();
    ...
    if (datamap_hit1.size()>5000){
        ...
        for (it_datamap_hit1=datamap_hit1.begin();it_datamap_hit1!=datamap_hit1.end();it_datamap_hit1++){
            ...
            CORR_t* hit=(CORR_t*)it_datamap_hit1->second;
            Long64_t corrts = 0;
            CORR_t* corrhit = 0;
            Long64_t ts1 = ts - 13000;
            ULong64_t ts2 = ts - 7000;
            ...
            while(it_datamap_hit2!=datamap_hit2.end()&&it_datamap_hit2->first<ts2){
                ...
                corrhit=(CORR_t*)it_datamap_hit2->second;
                hcorrcloverSi->Fill(corrts - ts);
                if (hit->dssd>=0)
                    he_e[hit->dssd]->Fill(hit->e,corrhit->e);
                if (hit->ch==0&&hit->dssd>=0){
                    he_e_strip_low[hit->dssd]->Fill(hit->e,corrhit->e);
                }
                if (hit->ch==31&&hit->dssd>=0){
                    he_e_strip_high[hit->dssd]->Fill(hit->e,corrhit->e);
                }
        ...
        for (it_datamap_hit2=datamap_hit2.begin();it_datamap_hit2!=datamap_hit2.end();it_datamap_hit2++){
            CORR_t* hit=(CORR_t*)it_datamap_hit2->second;
            delete hit;
        }
        datamap_hit1.clear();
        datamap_hit2.clear();
    }
    Double_t calib_parms0[]={5.,0.,5.5,5.};
    Double_t calib_parms1[]={0.158841159, 0.173391494,0.15392062,0.160606061};
    ...
    if (data_now->b == 12){
        ...
        Double_t e = hit->clong;
        ULong64_t ts = data_now->ts;
        if (hit->clong>100){
            //he1dclover[ch]->Fill(hit->clong*calib_parms1[ch-4]+calib_parms0[ch-4]);
        }
        if (ch>=4&&ch<8){//! only clover
            CORR_t* cloverhit=new CORR_t;
            cloverhit->e = e*calib_parms1[ch-4]+calib_parms0[ch-4];
            cloverhit->ch = ch;
            datamap_hit2.insert(make_pair(ts,cloverhit));
        }
    }
    if (data_now->b == 11) {
    ...
    if (data_now->b >= 0 && data_now->b <= 7) {
        ...
        if (data_now->b>=4){
            for (Int_t i=0;i<V1740_N_MAX_CH;i++){
                NIGIRIHit* hit=data_now->GetHit(i);
                Int_t ch = i + (data_now->b-4)*V1740_N_MAX_CH;
                Int_t itcnt= 0 ;
                if (hit->clong>0){
//                    for (std::vector<UShort_t>::iterator it =hit->pulse.begin() ; it != hit->pulse.end(); ++it){
//                        //if (itcnt<N_MAX_WF_LENGTH){
...
//                    }
        ...
        for (aidaIt = aida_data_map.begin(); aidaIt != aida_data_map.end(); aidaIt++) {
            ...
            if(aida_data.ID == 4){
                ...
                CORR_t* wasabihit=new CORR_t;
                wasabihit->e = aida_data.EX;
                wasabihit->ch = aida_data.x;
                wasabihit->dssd = detNum;
                datamap_hit1.insert(make_pair(aida_data.T,wasabihit));
                //cout<<aida_data.T<<endl;
            }
        }
    }
}
...
void CloseMe(){
    if (file0&&tree) {//! offline mode
        ...
        file0 = new TFile(Form("online_%s.root",tbuff),"recreate");
        for (int i = 0 ;i <4; i++) {
            detXYBeta[i]->Write();
            detExEyBeta[i]->Write();
            he1dclover[i]->Write();
            he_e[i]->Write();
            he_e_strip_low[i]->Write();
            he_e_strip_high[i]->Write();
        }
        file0->Close();
    }
...
const pmap_decode packetdecode[]={NONE,V1740ZSP,V1740ZSP,V1740ZSP,V1740ZSP,NONE,NONE,NONE,NONE,NONE,NONE,NONE,NONE,V1730DPPPHA};
